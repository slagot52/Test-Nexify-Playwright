# -*- coding: utf-8 -*-
"""
PLAYWRIGHT TEST - publicisnexify.com (JSON-driven Amazon DSP "Samsung Deal Open Video" suite)
================================================================================
...
what the UI actually lets you pick. Fifth Amazon JSON-driven suite in this
repo, after Mugler, Display, and Open Intereses - all three still live on
their own not-yet-merged branches/PRs, so (same lesson learned repeatedly)
this file is self-contained rather than importing from any of them.

Reference JSON: template_8315549690802_593484670914195308_AMAZON_SAMSUNG_DEAL_OPEN_VIDEO.json
(Samsung, Amazon advertiserId 8315549690802 / clientId 3635719820670341 -
same account CONFIRMED LIVE via the Display suite). 1 campaign, 3 ad
groups, all `inventoryType: ONLINE_VIDEO` (not DISPLAY-family) - the FIRST
Amazon suite where Video Completion Tier is actually exercised rather than
skipped (see DISPLAY_FAMILY_INVENTORY_TYPES below - ONLINE_VIDEO isn't in
that set). Campaign-level budgetAllocation is MANUAL here (Display/Open
Intereses were both AUTO) - the IO-level date fields don't render in MANUAL
mode, exercising that branch of create_insertion_order_amazon for the first
time live.

CLIENT/ADVERTISER = "Samsung"/"Samsung_ES_Starcom" - reused directly, no
longer a placeholder: CONFIRMED LIVE 2026-07-23 for this exact advertiserId
via the Display and Open Intereses suites' successful runs.

--------------------------------------------------------------------------
Cross-checked against the real backend validation schema before writing
(~/Downloads/schemas/amazonads/schema.json - see project memory
reference_backend_validation_schemas / feedback_check_schemas_before_writing_tests):
--------------------------------------------------------------------------
- `dailyMinSpendValue.minimum = 4.75` (confirmed, same floor as before) -
  filled with "5" here too, not "1".
- No `minimum` on baseBid or budgetValue anywhere in the schema, so the
  usual token "1" is safe for those.
- adGroup's `required` fields per the schema: adProduct, name, inventoryType,
  creativeRotationType, endDateTime, startDateTime, pacing - all already
  filled by this suite.
- `inventoryType` enum confirms `ONLINE_VIDEO` is a valid ad-group-level
  token (distinct from the campaign-level `primaryInventoryTypes` enum,
  which uses `VIDEO_OLV` instead - irrelevant here since
  primaryInventoryTypes has no UI control at all, per
  nexify-test-protect-1-26).
- No `if`/`then` conditional anywhere tying `videoCompletionTier` to
  `inventoryType` in this schema (confirmed zero conditionals in the Amazon
  schema besides `oneOf` for polymorphic fields) - the DISPLAY-family
  rejection found on the Display suite is a runtime/business rule the
  static schema doesn't express, not something schema-reading would have
  caught either way.

Budgets/bids stay at a token amount to avoid real spend (baseBid, IO flight
budget, ad-group budget, KPI Value all = 1; dailyMinSpendValue = 5, see
above). Frequency caps and every enum-style selection (Goal, KPI, Budget
Allocation, Delivery Profile, Viewability Tier, Video Completion Tier,
Inventory Type, Creative Rotation, ad-group Bid Strategy) ARE derived for
real from the JSON.

--------------------------------------------------------------------------
Multiple ad groups (3), reusing the "Create another" pattern from the Open
Intereses suite, but with a MORE GENERAL ad-group naming helper: Open
Intereses' 5 ad groups only differed in their trailing "_FF~<segment>" tag
(their "_PD~" segment was identically "AMAZON" for all 5), while this
export's 3 ad groups differ in "_PD~<name>" ("Intext-Gaming-V2", "Preroll -
Gaming", "Intext- Gaming") but all share "_FF~none". A single hardcoded
regex would only work for one export or the other - compute_ad_group_tags()
below tries "_PD~"/"_FF~" against the WHOLE list and picks whichever
segment is actually unique across all of them, falling back to a plain
index if neither is.
--------------------------------------------------------------------------

CONFIRMED UI GAP - Device targeting cannot be recreated at all (same as
every prior Amazon suite): all DEVICE targets use `deviceTarget.deviceType`
+ `mobileEnvironment` - the ad-group form's only "Device" section maps to
`deviceTarget.mobileDevice`, a completely different enum. NOTE'd per
target, not silently dropped.

Other confirmed gaps (present in the JSON, no UI control - NOTE'd, same as
prior suites): campaign-level `bidSettings.bidStrategy` (dead/unwired
control); ad-group-level `frequencies` (present on 1 of 3 ad groups here);
campaign-level `countries`/`marketplaces`;
`targetingSettings.defaultAudienceTargetingMatchType` (SIMILAR on 2 ad
groups, EXACT on the third - still no control either way); ad-group `fees`;
campaign-level `fees` (AGENCY).

Run with:        python test_amazon_samsung_deal_open_video_json_playwright.py
"""
...
# Constants
# --------------------------------------------------------------------------
REFERENCE_JSON = Path("/Users/k052/Downloads/template_8315549690802_593484670914195308_AMAZON_SAMSUNG_DEAL_OPEN_VIDEO.json")
...
# display-family ad group. None of this export's ad groups are in this set
# (all ONLINE_VIDEO), so this is the first suite where the field is
# actually filled rather than skipped.
...
# VALUE strings do NOT match what the live UI actually renders - these are
# the live-confirmed option labels.
...
def test_general_info(page: Page):
    ...
    campaign_name = f"Test Amazon Samsung Deal Open Video JSON - {int(time.time())}"
...
def create_insertion_order_amazon(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    order_name = f"Order Amazon Samsung Deal Open Video - {int(time.time())}"
    ...
    # IO-level dates only render in AUTO mode, hidden in MANUAL. This
    # export's campaign is MANUAL - first suite to actually exercise this
    # branch live (Display/Open Intereses were both AUTO).
    ...
    # spend, so safe to derive exactly). `or []` handles an explicit JSON
    ...
    if freqs:
        ...
        print("TEST io-frequency-caps SKIPPED -> campaign.frequencies is empty/null in this export")
...
# --------------------------------------------------------------------------
# Line Items / Ad Groups (this export has 3 - "Create another" between them)
# --------------------------------------------------------------------------
AD_GROUP_TAG_PATTERNS = [re.compile(r"_PD~(.+?)_BS~"), re.compile(r"_FF~(.+)$")]


def compute_ad_group_tags(ad_groups: list) -> list:
    """Extract a human-readable, DISTINGUISHING tag per ad group for
    suite-generated naming. Different exports vary in which name segment
    actually differs across ad groups (Open Intereses: "_FF~<segment>" was
    unique per ad group but "_PD~" was identically "AMAZON" for all 5; this
    export is the opposite - "_PD~<name>" differs, "_FF~none" doesn't) - try
    each candidate pattern against the WHOLE list and use the first one that
    yields a non-empty, unique tag for every ad group. Falls back to a plain
    index if neither pattern disambiguates."""
    names = [ag.get("name", "") for ag in ad_groups]
...
def _fill_single_ad_group(page: Page, ad_form, ad_group: dict, date_from: datetime.date, date_to: datetime.date, index: int, tag: str):
    ad_group_name = f"AG Samsung Deal Open Video {tag} - {int(time.time())}"
    ...
    # CONFIRMED LIVE 2026-07-23 (Display suite) and cross-checked against
    # amazonads/schema.json's dailyMinSpendValue.minimum=4.75: the "1" token
    # every other spend-adjacent field uses is too low for this specific
    # field, so this suite uses "5" instead.
    fill_and_verify(ad_form, "dailyMinSpendValue", "5")
    ok(f"ag{index}-daily-min-spend", "Daily Min Spend = 5 (token, above the confirmed live/schema minimum of 4.75)")
    ...
    # display-family ad group. This export's ad groups are all ONLINE_VIDEO
    # (not in DISPLAY_FAMILY_INVENTORY_TYPES) - the FIRST Amazon suite where
    # this field is actually filled rather than skipped.
    video_completion = ad_group["targetingSettings"].get("videoCompletionTier")
    if video_completion and inventory_type not in DISPLAY_FAMILY_INVENTORY_TYPES:
        ...
        print(f"TEST ag{index}-video-completion SKIPPED -> inventoryType '{inventory_type}' does not accept "
              ...
              "if set for DISPLAY-family types - a real Nexify product bug, not fixed here)")
    ...
    # (same id for all 3 ad groups in this export).
    ...
    # export wants MULTIPLE_SIGNALS (unchecked) for every ad group, same as
    # the Display suite's case that first required the force-click-on-native-
    # input fix.
    ...
    if len(ad_group.get("budgets", [])) > 1:
        print(f"TEST ag{index}-budget-segments NOTE -> {len(ad_group['budgets'])} budget segments in the JSON "
              "collapse into the form's single Lifetime-only budget field")
    ...
    # Date should be within campaign dates" - the suite's own verification
    ...
    if not start_date_input.input_value().strip() or not end_date_input.input_value().strip():
        ...
        expect(start_date_input).not_to_have_value("")
        expect(end_date_input).not_to_have_value("")
        print(f"NOTE ag{index}-dates: end/start date input was reset by a debounced re-render after adding the "
...
def build_ad_groups_amazon(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    for i, ad_group in enumerate(ad_groups):
        if i > 0:
            ...
            # slate, NOT "Duplicate"). The form rebinds automatically to the
            # new active ad group (hydrateEffect watches activeAdGroup()).
            create_btn = ad_groups_section.locator("button", has_text="Create another")
            create_btn.scroll_into_view_if_needed()
            create_btn.click()
            page.wait_for_timeout(500)
        name = _fill_single_ad_group(page, ad_form, ad_group, date_from, date_to, i, tags[i])
        names.append(name)
...
def finish_and_submit_amazon(page: Page):
    """Same after-submit logic as the other Amazon suites: plain
