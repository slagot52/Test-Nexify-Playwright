# -*- coding: utf-8 -*-
"""
PLAYWRIGHT TEST - publicisnexify.com (JSON-driven TTD "Samsung Programmatic Guarantees" suite)
================================================================================
FIRST JSON-driven TTD suite in this repo (no `nexify-ttd-json-suite` skill/
playbook exists yet, unlike DV360/Amazon). Checks that TTD campaign/ad-group
targeting in Nexify can be driven from a real export, comparing the export's
values against what the UI actually lets you pick.

Reference JSON: template_v2_mh1ol0m_TTD_SAMSUNG_PROGRAMMATIC_GUARANTEES.json
(Samsung, TTD AdvertiserId "ihee7uv" - part of the same 2026-07-23 Samsung
batch as the 3 Amazon suites; see project memory
project_samsung_amazon_ttd_batch). 1 campaign ("Channel"), 2 ad groups.

>>> CLIENT / ADVERTISER = "Samsung" / "Samsung_ES_Starcom" - UNVERIFIED
>>> PLACEHOLDER GUESSES, MORE UNCERTAIN than the Amazon suites' equivalent
>>> guess. TTD's AdvertiserId here ("ihee7uv") is an opaque TTD-native id
>>> with no numeric overlap with the Amazon/DV360 Samsung advertiserIds, so
>>> there is no partial-id correlation to lean on - this is purely a
>>> filename-based guess ("...SAMSUNG..."). Verify against the advertiser
>>> search grid (TTD badge) on General Info and correct if wrong.

--------------------------------------------------------------------------
CRITICAL STRUCTURAL NOTE: this export is NOT shaped like Nexify's own
payload (unlike the DV360/Amazon exports, which are close to
payload.campaigns/adGroups already). It is TTD's OWN raw native "Campaign"
API object verbatim (CampaignFlights, RTBAttributes, KoaOptimizationSettings,
NielsenSettings, ComscoreSettings, ContractTargeting, etc. are literal TTD
API concepts, not Nexify UI concepts) - confirmed by comparing its top-level
keys against ~/Downloads/schemas/ttd/schema.json, whose top-level required
is `["CampaignGlobal", "CampaignChannels"]` - a COMPLETELY different wrapper
shape than this file's top-level keys (`Budget`, `PacingMode`, `AdGroups`,
etc. directly). So this suite is a genuine field-MAPPING exercise (raw TTD
object -> Nexify UI controls), not a payload-reuse exercise.
...
(AudienceTargeting, KoaOptimizationSettings, NielsenSettings,
ComscoreSettings, ContractTargeting, DimensionalBiddingAutoOptimizationSettings,
QualityAllianceViewabilityProfile, viewability-standard fields, etc.) that
baseline itself leaves at defaults / doesn't exercise. Everything outside
that proven surface is NOTE'd as out-of-scope, not silently dropped.
Imports `_open_and_select` / `_set_date_range_dialog` directly from
test_ttd_playwright.py (frozen but safe to import from, confirmed present
on `main`) rather than re-deriving them.
...
feedback_check_schemas_before_writing_tests):
--------------------------------------------------------------------------
...
- No `if`/`then` conditionals in this schema at all (confirmed via grep) -
  cross-field business rules (if any exist for TTD) aren't expressible here
  and wouldn't be caught by schema-reading regardless.

--------------------------------------------------------------------------
Field mapping notes (raw TTD JSON -> Nexify TTD UI, derived from
nexify-frontend-main source, not guessed):
--------------------------------------------------------------------------
- Campaign/Channel Name: SYNTHETIC (same convention as every other suite's
  Order/Insertion-Order name), not the JSON's real `CampaignName` - that
  real value is reserved for General Info's own synthetic wrapper name.
- `PacingMode` ("PaceAhead"): `TtdPacingMode` enum, KEY=API token,
  VALUE=label ("Pace Ahead") - `ttd-pacing-mode.ts`.
- `PrimaryChannel`/AdGroup `ChannelId` ("TV"): TWO SEPARATE enums
  (`TtdPrimaryChannel`/`TtdChannelId`) that happen to both use "TV"/"TV" for
  this token, but differ elsewhere (e.g. Out of Home is spelled
  differently) - kept as two separate label dicts, not merged.
- `PrimaryGoal.VCRInPercent` (80.0): the PRESENCE of this key (not its
  value) selects Goal Type = "Completion Rate" (`goalTypeLabel` dict,
  `ttd-campaign-channels.component.ts`) - confirmed `VCRInPercent` is
  allowed for channel "TV" (`goalTypeAllowedChannels.VCRInPercent` includes
  'TV'). Goal % itself is token "1" (not the real 80.0), same
  budgets/bids/KPI-value-token convention every suite uses.
- `TimeZone` ("Etc/GMT", no offset): does NOT match any entry in
  `TtdTimezonesMapping` (which only has offset variants like "Etc/GMT+12"),
  nor any enum key/label directly - confirmed via
  `ttd-global-setup.component.ts`'s own `resolveTimeZoneKey()` fallback
  chain, an unmatched value silently defaults to `'Etc_UTC'` = "(UTC)
  Coordinated Universal Time" - the exact same choice the baseline suite
  already uses. Not a guess: traced through the frontend's own fallback
  logic.
- `SeedId` ("o6g215d5"): opaque id, no display name in the JSON - resolved
  live the same way Amazon/DV360 resolve raw ids: capture the
  `/dsp/ttd/seeds` response (auto-fetched when Global Setup loads, no
  search/button needed) and match by id.
- `PurchaseOrderNumber`: SYNTHETIC random 8-digit number, matching the
  baseline's own generation approach - the JSON's real value
  ("95 26 7770, 95 26 7773") doesn't fit that format anyway.
- `CampaignConversionReportingColumns` (2 real columns): maps to the
  Conversion Reporting dialog's Cross-Device Concept + Vendor per row -
  DERIVED FOR REAL, not the baseline's one hardcoded example. Concept
  (Person/Household) is inferred from whether the JSON's
  `CrossDeviceAttributionModelId` token contains "household"
  (case-insensitive) - mirrors `inferConcept()` in
  `ttd-conversion-reporting-dialog.component.ts` exactly, not guessed.
  Vendor id -> display name resolved live via the
  `/dsp/ttd/crossDeviceAttributionVendor` response (fetched automatically
  when the dialog's "Configure" button is clicked). `ReportingColumnId`/
  `TrackingTagId` are left at their auto-assigned defaults, same as the
  baseline (which never touches those two selects either).
- AdGroup `IsEnabled` (true on both ad groups): maps to the optional
  "Enabled" checkbox, same present-on-some-accounts handling as baseline.
- Two ad groups here ("HBO"/"NETFLIX" per their `_PD~` name segment) use
  the same "Create another" + auto-rebinding-form multi-ad-group pattern
  already proven on the Amazon suites (`TtdAdGroupsStore.add()` behaves
  identically to `AmazonAdGroupsStore.add()` - confirmed by reading both
  stores).

Out-of-scope / NOTE'd, not fixed here (no control in the baseline's proven
surface): AdGroup-level `RTBAttributes.BudgetSettings` (no ad-group-level
budget control at all - confirmed via grep of `ttd-ad-groups.component.html`,
only the Campaign Channel's Flight budget is settable),
`RTBAttributes.AudienceTargeting` (baseline's own "Audience: Target
Everyone" default), `RTBAttributes.ROIGoal` (ad-group-level goal, no
distinct control found), `RTBAttributes.CreativeIds` (creative/ads
out-of-scope, same precedent as every DSP), KoaOptimizationSettings /
NielsenSettings / ComscoreSettings / ContractTargeting /
DimensionalBiddingAutoOptimizationSettings / the three viewability-standard
fields, campaign-level `AssociatedBidLists` / `Increments` / `CustomLabels`
(campaign and ad-group level) / `CampaignConversionReportingColumns`'
`TrackingTagId`/`ReportingColumnId` sub-fields, `CustomCPAClickWeight`/
`CustomROASType`, `Description` (null anyway), `IsBallotMeasure`,
`CampaignType`.

Run with:        python test_ttd_samsung_programmatic_guarantees_json_playwright.py
"""
...
# Constants
# --------------------------------------------------------------------------
REFERENCE_JSON = Path("/Users/k052/Downloads/template_v2_mh1ol0m_TTD_SAMSUNG_PROGRAMMATIC_GUARANTEES.json")
...
# JSON verbatim), VALUE = the visible mat-option label.
# --------------------------------------------------------------------------
PACING_MODE_LABELS = {
...
def test_general_info(page: Page):
    ...
    campaign_name = f"Test TTD Samsung Programmatic Guarantees JSON - {int(time.time())}"
...
def test_global_setup(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    # ("95 26 7770, 95 26 7773") doesn't fit that format.
    ...
    # fallback logic, to the same default the baseline suite already uses.
...
def test_campaign_channels(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    channel_name = f"Channel Samsung PG - {int(time.time())}"
    ...
    # Flight - this export has 3 real (long-expired) flights; only ONE
...
# --------------------------------------------------------------------------
# Ad Groups (this export has 2 - "Create another" between them, same
# multi-ad-group pattern already proven on the Amazon suites)
# --------------------------------------------------------------------------
AD_GROUP_TAG_PATTERNS = [re.compile(r"_PD~(.+?)_BS~"), re.compile(r"_FF~(.+)$")]


def compute_ad_group_tags(ad_groups: list) -> list:
    """Same generalized tag-extraction helper as the Amazon Deal Open Video
    suite: try each candidate name-segment pattern against the WHOLE
    ad-group list and use the first one that's unique for every ad group,
    falling back to a plain index if neither disambiguates."""
...
def _fill_single_ad_group(page: Page, ag_form, ad_group: dict, index: int, tag: str):
    ad_group_name = f"AG Samsung PG {tag} - {int(time.time())}"
...
def test_recap(page: Page, campaign_name: str):
    """Same after-submit logic as the baseline TTD suite: a summary dialog
    may reappear here too, dismissed best-effort, then the Start-campaign
    gate with a real launch requiring a typed 'yes'."""
    page.locator("div.step-footer").locator("button.mdc-button", has_text="Next").click()
    dialog = page.locator("dv360-io-summary-dialog")
    try:
