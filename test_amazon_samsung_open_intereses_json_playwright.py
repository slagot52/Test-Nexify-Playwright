...
def _fill_single_ad_group(page: Page, ad_form, ad_group: dict, date_from: datetime.date, date_to: datetime.date, index: int):
    ...
    # test_amazon_playwright.py). CONFIRMED LIVE 2026-07-23: a real Amazon
    # DSP submission failed with "End Date should be within campaign dates"
    # - this suite's own verification only ever re-checked the START date
    # input for emptiness, never the END date input, so a wiped/incorrect
    # end date would sail through undetected. Both readonly display inputs
    # (Start Date/Time, End Date/Time - amazon-ad-groups.component.html)
    # come from the SAME shared date-range dialog, but are now checked and
    # retried symmetrically below.
