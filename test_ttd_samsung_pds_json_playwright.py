# -*- coding: utf-8 -*-
"""
PLAYWRIGHT TEST - publicisnexify.com (JSON-driven TTD "Samsung PDs" suite)
================================================================================
Second JSON-driven TTD suite in this repo, built directly from the field
mappings/playbook established by the first one (Samsung Programmatic
Guarantees - see project memory project_ttd_samsung_programmatic_guarantees_suite).
Checks that TTD campaign/ad-group targeting in Nexify can be driven from a
real export, comparing the export's values against what the UI actually
lets you pick.

Reference JSON: template_v2_yui6h9h_TTD_SAMSUNG_PDs.json (same Samsung/TTD
AdvertiserId "ihee7uv" as the Programmatic Guarantees export - part of the
same 2026-07-23 Samsung batch as the 3 Amazon suites and the PG suite; see
project memory project_samsung_amazon_ttd_batch). 1 campaign ("Channel"),
3 ad groups.

>>> CLIENT / ADVERTISER = "Samsung" / "Samsung_ES_Starcom" - CONFIRMED, not
>>> a placeholder guess this time. Same AdvertiserId ("ihee7uv") as the PG
>>> suite, whose Seed Id resolved live to a real seed named
>>> 'ES_1stP_Starcom' for this exact advertiser - reusing that confirmation
>>> here rather than re-guessing from the filename.

--------------------------------------------------------------------------
CRITICAL STRUCTURAL NOTE (same as the PG suite): this export is NOT shaped
like Nexify's own payload. It is TTD's OWN raw native "Campaign" API object
verbatim (CampaignFlights, RTBAttributes, KoaOptimizationSettings,
NielsenSettings, ComscoreSettings, ContractTargeting, etc. are literal TTD
API concepts, not Nexify UI concepts). Same genuine field-MAPPING exercise
as before, not a payload-reuse exercise - and since it's the same
advertiser/account, almost every enum/mapping decision below carries over
unchanged from the PG suite.
...
the baseline itself leaves at defaults / doesn't exercise. Everything
outside that proven surface is NOTE'd as out-of-scope, not silently
dropped. Imports `_open_and_select` / `_set_date_range_dialog` directly
from test_ttd_playwright.py (frozen but safe to import from, confirmed
present on `main`) rather than re-deriving them.
...
result as when the PG suite checked it:
--------------------------------------------------------------------------
...
- No `if`/`then` conditionals in this schema at all (confirmed via grep,
  same as the PG suite's check) - cross-field business rules (if any exist
  for TTD) aren't expressible here and wouldn't be caught by schema-reading
  regardless.

--------------------------------------------------------------------------
Field mapping notes (raw TTD JSON -> Nexify TTD UI). Same account as the PG
suite, so every enum token seen here was already resolved there - no new
label-dict entries needed:
--------------------------------------------------------------------------
- Campaign/Channel Name: SYNTHETIC, same convention as every other suite.
- `PacingMode` ("PaceAhead") -> "Pace Ahead", same as the PG suite.
- `PrimaryChannel` ("Video") -> `PRIMARY_CHANNEL_LABELS["Video"]` = "Video".
- AdGroup `ChannelId` ("Video" on all 3 ad groups) ->
  `CHANNEL_ID_LABELS["Video"]` = "Video".
- `PrimaryGoal.VCRInPercent` (80.0, identical shape to the PG suite): the
  PRESENCE of this key selects Goal Type = "Completion Rate". Goal % itself
  is token "1" (not the real 80.0), same convention.
- `FunnelLocation` ("Awareness" on all 3 ad groups) ->
  `FUNNEL_LOCATION_LABELS["Awareness"]` = "Awareness".
- `TimeZone` ("Etc/GMT", no offset): same as the PG suite - resolves via
  `resolveTimeZoneKey()`'s own fallback to `Etc_UTC` = "(UTC) Coordinated
  Universal Time".
- `SeedId` ("o6g215d5"): IDENTICAL id to the PG suite's export (same
  advertiser/account) - resolved live the same way, via the `/dsp/ttd/seeds`
  response captured on Global Setup load. Already confirmed live to be
  'ES_1stP_Starcom' by the PG suite's own run.
- `PurchaseOrderNumber`: SYNTHETIC random 8-digit number - the JSON's real
  value ("95 26 7779") doesn't fit that format anyway.
- `CampaignConversionReportingColumns` (12 entries, SAME SHAPE as the PG
  suite): collapses to exactly 2 distinct non-null `CrossDeviceAttributionModelId`
  values - "IdentityAlliance" and "IdentityAllianceWithHousehold" - via the
  same first-occurrence-per-distinct-vendor-id dedup the PG suite already
  uses. Concept (Person/Household) inferred the same way
  (`_infer_concept()`, unchanged) - "IdentityAllianceWithHousehold" contains
  "household" case-insensitively -> Household, "IdentityAlliance" -> Person.
- AdGroup `IsEnabled` (true on all 3 ad groups): optional "Enabled"
  checkbox, same present-on-some-accounts handling as the PG suite.
- **THREE ad groups here, not two** - and unlike the PG suite, all three
  `AdGroupName` values are byte-for-byte IDENTICAL except for their
  trailing `_FF~` segment ("olv"/"olvV2"/"olvW"). The `_PD~(.+?)_BS~`
  pattern `compute_ad_group_tags()` tries first yields the SAME tag
  ("prerollallpublisher") for all three, so it falls through to the
  `_FF~(.+)$` pattern, which IS unique here - this is the first real
  live exercise of that fallback (the PG suite's own 2 ad groups matched
  on the first pattern). No code change needed - `compute_ad_group_tags()`
  already tries both patterns generically.
- `RTBAttributes.BudgetSettings` / `RTBAttributes.AudienceTargeting` are
  present (non-empty) on all 3 ad groups here too - same NOTE'd
  out-of-scope handling as the PG suite (no ad-group-level budget control
  in this frontend at all; audience left at default "Target Everyone").
- Multi-ad-group looping: same "Create another" + auto-rebinding-form
  pattern already proven on the PG and Amazon suites.

Out-of-scope / NOTE'd, not fixed here (no control in the baseline's proven
surface, identical list to the PG suite): AdGroup-level
...
Run with:        python test_ttd_samsung_pds_json_playwright.py
"""
...
# Constants
# --------------------------------------------------------------------------
REFERENCE_JSON = Path("/Users/k052/Downloads/template_v2_yui6h9h_TTD_SAMSUNG_PDs.json")
CLIENT = "Samsung"                 # CONFIRMED live by the PG suite (same AdvertiserId)
ADVERTISER = "Samsung_ES_Starcom"  # CONFIRMED live by the PG suite (same AdvertiserId)
TTD_DSP_BADGE = "TTD"
...
# JSON verbatim), VALUE = the visible mat-option label. Identical to the PG
# suite's dicts - same account, same enums encountered.
# --------------------------------------------------------------------------
PACING_MODE_LABELS = {
...
def test_general_info(page: Page):
    ...
    campaign_name = f"Test TTD Samsung PDs JSON - {int(time.time())}"
...
def test_global_setup(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    # action needed). Same id as the PG suite's export - expect the same
    # 'ES_1stP_Starcom' match.
    if seed_id:
    ...
    # ("95 26 7779") doesn't fit that format.
    ...
    # fallback logic, to the same default the baseline/PG suites already use.
...
def fill_conversion_reporting(page: Page, cc_form, columns: list):
    ...
    # The PG suite (same advertiser) hit this live every time - expect the
    # same here. Only crossDeviceAttributionVendor's response body is
    # actually needed - hard-waiting on trackingTags too crashed with an
    # unhandled Playwright TimeoutError in the PG suite's own debugging, so
    # that second wait is deliberately NOT repeated here.
    ...
    try:
        ...
        # CONFIRMED LIVE on the PG suite (same advertiser) 2026-07-23: the
        # dialog never opens for this advertiser - crossDeviceAttributionVendor
        # resolves fine (200) but getAdvertiserTrackingTags never produces an
        # observable response, and openConversionReportingDialog() has no
        # catch around their Promise.all, so the function hangs forever
        # mid-await and dialog.open() is never reached. A real Nexify
        # product bug (see user_nexify_developer), not something this suite
        # can work around by clicking through it differently - there's no
        # dialog to dismiss, just a permanently "Loading..." button.
        # Degrade gracefully: NOTE it and move on rather than failing the
        # whole run over one unconfigurable, optional section.
        still_loading = conv_section.get_by_text("Loading...", exact=True).count() > 0
        print("NOTE: CONFIRMED PRODUCT BUG (same as the PG suite, same advertiser) - the Conversion "
...
def test_campaign_channels(page: Page, ref: dict, date_from: datetime.date, date_to: datetime.date):
    ...
    channel_name = f"Channel Samsung PDs - {int(time.time())}"
    ...
    # precedent as the baseline/PG suites.


# --------------------------------------------------------------------------
# Ad Groups (this export has 3 - "Create another" between them, same
# multi-ad-group pattern already proven on the PG/Amazon suites)
# --------------------------------------------------------------------------
AD_GROUP_TAG_PATTERNS = [re.compile(r"_PD~(.+?)_BS~"), re.compile(r"_FF~(.+)$")]


def compute_ad_group_tags(ad_groups: list) -> list:
    """Same generalized tag-extraction helper as the PG/Amazon Deal Open
    ...
    falling back to a plain index if neither disambiguates. This export's 3
    AdGroupNames are identical on their `_PD~` segment ("prerollallpublisher")
    - the first real live exercise of the `_FF~` fallback (which IS unique
    here: "olv"/"olvV2"/"olvW")."""
...
def _fill_single_ad_group(page: Page, ag_form, ad_group: dict, index: int, tag: str):
    ad_group_name = f"AG Samsung PDs {tag} - {int(time.time())}"
...
def test_recap(page: Page, campaign_name: str):
    """Same after-submit logic as the baseline/PG TTD suites: a summary
