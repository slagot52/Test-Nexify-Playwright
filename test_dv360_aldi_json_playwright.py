...
def install_submit_guard_no_block(page: Page):
    ...
    request - it still dumps the payload and prints the missing-bid-value
    diagnostic to the console (terminal only, no page interaction), but
    always lets the request continue through to the real backend regardless
    of the result."""
    ...
    def _route(route):
        ...
        if missing:
            ...
            print("[submit-guard] NOT blocking (per user request) - YouTube ad groups missing a "
                  "positive bid `value`, sending to the backend anyway:")
            for m in missing:
                print(f"    - {m}")
        else:
